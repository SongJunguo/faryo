...
bash install-faryo.sh --version v1.6.1 --workspace /path/to/workspace
```

The script then downloads `faryo-v1.6.1.tar.gz` and its checksum, accepts only a
...
```bash
bash install-faryo.sh --version v1.6.1 --workspace /path/to/workspace --migrate-owner
```
...
faryo update --version v1.6.1   # exact release
...
faryo update --version v1.6.1 \
  --archive ./faryo-v1.6.1.tar.gz \
  --checksum ./faryo-v1.6.1.tar.gz.sha256
```
