...
- 原 `backup/pre-deepseek-ui-20260815`、`katex-feature` 和功能分支在完整提升到 `main`
  后已删除；标签保留同一旧基线，不依赖长期冗余分支。
...
- `89eb9aa`：统一 AST/KaTeX 管线、Shiki 懒加载、稳定块 DOM 对账、旧实现删除及
  Live 滚动竞态修复。
- 用户已授权进行大范围重构，并明确“效果和长期可维护性第一”；实施期间由开发分支
  与固定标签承担回滚，完成后内容提升到 `main`，生产代码不长期保留旧同功能实现。
- 阶段 D 的实现与发布回归已完成：单一 AST 渲染核心、Shiki 按需高亮、稳定块 DOM
  对账和 256 项有界内存缓存已接入，旧 Markdown/数学双实现已从运行时代码删除。
- 阶段 E/F/G 已完成：匿名精确内容提交矩阵、附件上传、断网/后台恢复、Gateway
  手机/桌面分页、认证边界、公网 Access、隐私扫描和用户观感验收均通过。
...
- 长对话新增右侧问题导航轨道：稳定用户块映射为小格标记，快速滚动时临时出现并自动
  隐藏，点击或键盘方向键可跳转；滚动显示时桌面/手机面板背景分别为 58%/52%，进入实际
  操作状态才提升到 90%，避免遮挡正文；问题预览只存在于当前 DOM，不写入浏览器存储。
...
4. 不修改 tmux 窗口尺寸、Codex TUI 宽度或终端自动换行行为。
...
- 正常阅读时轨道完全透明且不接收指针事件；仅用户快速滚轮/滑动触发短暂显示，停止后
  1.4 秒自动隐藏；滚动态使用半透明低模糊面板，hover、触摸交互或键盘 focus 才恢复高
  对比度。程序化刷新/自动滚到底部不触发。
- 当前问题按主滚动容器的阅读锚点计算；位于底部时明确选中最后一问。
- 点击标记平滑跳到对应问题，键盘支持方向键、Home、End；桌面 hover/键盘 focus 显示
  当前页内截断预览。
- 手机将轨道作为最右侧临时浮层，不预留永久空白也不改变正文宽度；桌面把轨道放在
  748 px 阅读主轴之外。显示与隐藏均不引发公式或正文重排。
...
- 390x844 与 1440x900 匿名 Chrome 均通过 13 问轨道、跳到第 8 问、预览、实时追加、
  阅读位置保持、滚动态半透明和滚到底部选中第 13 问；页面无横向溢出，临时 tmux 尺寸
  不变。
...
- 390x844 Chrome 与 1440x900 Microsoft Edge 匿名 13 问浏览器回归通过：平时隐藏、
  快速滚动显示、1.4 秒自动隐藏、Tab 进入轨道、方向键跳转、点击第 8 问，以及追加第
  13 问时主滚动位置变化不超过 2 px；旧标记 DOM 保持复用，轨道出现前后正文宽度差
  不超过 1 px。
...
回滚只需从固定标签创建新分支；已删除的旧开发分支无需恢复，也不向原作者
`upstream` 推送任何内容。
...
- 完整功能线提升到个人 `origin/main`，固定回滚标签仍可访问，原作者 `upstream` 未被修改。

## 10. 完成审计（截至 2026-08-19）
