...
test "$(curl --noproxy '*' -sS -o /dev/null -w '%{http_code}' "http://$FARYO_OWNER_HOST:$FARYO_OWNER_PORT/pet/pet-idle.png")" = "404"
...
test "$(curl --noproxy '*' -sS -o /dev/null -w '%{http_code}' "http://$FARYO_OWNER_HOST:$FARYO_OWNER_PORT/compact-rules-claude.js")" = "404"
...
grep -q 'DeepSeek Harness-derived' "$TMP"
test "$(curl --noproxy '*' -sS -o "$TMP" -w '%{http_code}' "http://$FARYO_OWNER_HOST:$FARYO_OWNER_PORT/vendor/markdown-ast/highlight/highlight.js")" = "200"
grep -q 'faryo-markdown-highlighter-ready' "$TMP"
test "$(curl --noproxy '*' -sS -o "$TMP" -w '%{http_code}' "http://$FARYO_OWNER_HOST:$FARYO_OWNER_PORT/vendor/markdown-ast/highlight/manifest.json")" = "200"
grep -q 'highlight/highlight.js' "$TMP"
