#!/usr/bin/env bash
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=_lib.sh
source "$SCRIPT_DIR/_lib.sh"
load_env
cd "$FARYO_OWNER_ROOT/local-tmux-owner"
exec env FARYO_OWNER_TOKEN="$FARYO_OWNER_TOKEN" \
  FARYO_OWNER_LABEL="$FARYO_OWNER_LABEL" \
  FARYO_OWNER_DATA="$FARYO_OWNER_DATA" \
  FARYO_OWNER_INBOX_DIR="$FARYO_OWNER_INBOX_DIR" \
  FARYO_OWNER_ARTIFACTS_DIR="$FARYO_OWNER_ARTIFACTS_DIR" \
  FARYO_OWNER_CACHE_DIR="$FARYO_OWNER_CACHE_DIR" \
  FARYO_OWNER_LOGS_DIR="$FARYO_OWNER_LOGS_DIR" \
  FARYO_OWNER_FILE_INBOX="$FARYO_OWNER_FILE_INBOX" \
  FARYO_PROJECT_WORKBENCH_ENABLE="${FARYO_PROJECT_WORKBENCH_ENABLE:-1}" \
  FARYO_PROJECT_WORKBENCH_GATEWAY_URL="${FARYO_PROJECT_WORKBENCH_GATEWAY_URL:-}" \
  FARYO_PROJECT_WORKBENCH_SYNC_URL="${FARYO_PROJECT_WORKBENCH_SYNC_URL:-}" \
  FARYO_PROJECT_WORKBENCH_SYNC_OWNER_LABEL="${FARYO_PROJECT_WORKBENCH_SYNC_OWNER_LABEL:-}" \
  FARYO_PROJECT_WORKBENCH_PROJECTS_ROOT="${FARYO_PROJECT_WORKBENCH_PROJECTS_ROOT:-}" \
  FARYO_PROJECT_WORKBENCH_ALLOWED_ROOTS="${FARYO_PROJECT_WORKBENCH_ALLOWED_ROOTS:-}" \
  FARYO_CODEX_BIN="${FARYO_CODEX_BIN:-}" \
  FARYO_AGENT_SHELL="${FARYO_AGENT_SHELL:-}" \
  FARYO_START_DIRECTORY_ROOTS="${FARYO_START_DIRECTORY_ROOTS:-}" \
  PATH="$HOME/.local/share/npm-global/bin:$HOME/.local/bin:/usr/local/bin:$PATH" \
  "$FARYO_PYTHON" server.py --session "$FARYO_OWNER_DIRECT_SESSION" --host "$FARYO_OWNER_HOST" --port "$FARYO_OWNER_PORT"
