...
OWNER_STATIC_FILES = {"appearance.css", "appearance.js", "app.js", "style.css", "index.html", "event-stream.js", "internal-annotations.js", "local-file-view.js", "stable-blocks.js", "question-navigator.js", "live-scroll.js", "compact-rules-codex.js"}
...
class GatewayHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/workbench":
            try:
                history_page = max(1, int(parse_qs(parsed.query).get("page", ["1"])[0]))
            except ValueError:
                history_page = 1
            self.write_json(self.workbench_payload(username, history_page), HTTPStatus.OK)
            return
    ...
    def handle_agent_new(self, username: str) -> None:
        try:
            payload = self.read_json_body(4096); route = str(payload.get("route") or "").strip(); command = clean_agent_launch_command(str(payload.get("command") or "")); requested_cwd = str(payload.get("cwd") or "").strip().rstrip("/"); cwd_token = str(payload.get("cwd_token") or payload.get("cwdToken") or "").strip()
...
const WORKBENCH_CACHE_KEY='faryoWorkbenchSnapshot';const labels=__LABELS_JS__;let draggedPackage=null;let assetTargetPackage=null;let handoffTargets=[];let actionBusy=false;let historyPage=1,historyTotalPages=1;
...
async function directoryPage(route,path){const query=path?`?path=${encodeURIComponent(path)}`:'',res=await fetch(`/${route}/api/directories${query}`,{cache:'no-store'}),data=await res.json();if(!res.ok||!data.ok)throw new Error(data.error||'Failed to list directories');return data;}
async function selectNewCwd(route,label,cwdChoices){const recent=Array.isArray(cwdChoices?.[route])?cwdChoices[route]:[];let path=String(recent[0]?.value||''),initial=true;while(true){let data;try{data=await directoryPage(route,path);}catch(error){if(initial&&path){path='';initial=false;continue;}throw error;}initial=false;const seen=new Set([data.path]),choices=[{label:'Use this folder',meta:data.displayPath||data.path,value:'__select__'}];if(data.parent){choices.push({label:'↑ Parent folder',meta:data.parentDisplayPath||data.parent,value:`path:${data.parent}`});seen.add(data.parent);}for(const item of data.roots||[]){if(!item.path||seen.has(item.path))continue;seen.add(item.path);choices.push({label:`Root · ${item.displayPath||item.path}`,meta:item.path,value:`path:${item.path}`});}for(const item of recent.slice(0,6)){const value=String(item.value||'');if(!value||seen.has(value))continue;seen.add(value);choices.push({label:`Recent · ${item.label||item.path}`,meta:item.path||value,value:`path:${value}`});}for(const item of data.directories||[]){if(!item.path||seen.has(item.path))continue;seen.add(item.path);choices.push({label:`Folder · ${item.name}`,meta:item.displayPath||item.path,value:`path:${item.path}`});}const selected=await sheet('Choose working directory',`${label} · ${data.displayPath||data.path}`,choices);if(selected===null)return null;if(selected==='__select__')return{cwd:String(data.path||''),cwdToken:String(data.selectionToken||'')};path=String(selected).replace(/^path:/,'');}}
...
function renderWorkbench(data){markRoutes(data.entries||[]);const packages=data.inbox||data.packages||[],rawSessions=data.sessions||[],activeSessions=Array.isArray(data.activeSessions)?data.activeSessions:rawSessions.filter(item=>item.tmuxSession),sessions=Array.isArray(data.activeSessions)?rawSessions:rawSessions.filter(item=>!item.tmuxSession),history=data.history||{},entries=data.entries||[],cwdChoices=data.agentCwdChoices||{},pkg=packages[0],packageItems=pkg?[pkg]:[],allowedCommands=new Set(data.newSessionCommands||['codex']),launchers=[{id:'new-codex',command:'codex',label:'Codex',entries,cwdChoices}].filter(item=>allowedCommands.has(item.command));const seenTargets=new Set();handoffTargets=[...activeSessions,...sessions].filter(item=>{const key=`${item.route}:${item.id||item.tmuxSession||''}`;if(!item.route||seenTargets.has(key))return false;seenTargets.add(key);return true;});historyPage=Math.max(1,Number(history.page||historyPage||1));historyTotalPages=Math.max(1,Number(history.totalPages||1));const historyPageInput=document.getElementById('historyPageInput');if(historyPageInput){historyPageInput.max=String(historyTotalPages);if(document.activeElement!==historyPageInput)historyPageInput.value=String(historyPage);}document.getElementById('packageCount').textContent=pkg?(pkg.status==='pending'?'· Ready':'· Sent'):'· Empty';document.getElementById('activeSessionCount').textContent=`${activeSessions.length} live`;document.getElementById('historyCount').textContent=`${Number(history.total??sessions.length)} total`;document.getElementById('historyPageTotal').textContent=String(historyTotalPages);document.getElementById('historyPrev').disabled=history.hasPrevious===false||historyPage<=1;document.getElementById('historyNext').disabled=history.hasNext===false||historyPage>=historyTotalPages;syncChildren(document.getElementById('packageList'),packageItems,item=>`pkg-${item.id}`,packageCard,'Choose files, then send them to a session.');syncChildren(document.getElementById('newSessionSlot'),launchers,item=>item.id,newAgentCard,'No launchers available');syncChildren(document.getElementById('activeSessionList'),activeSessions,item=>`active-${item.route}-${item.tmuxSession||item.id}`,sessionCard,'No active agent sessions');syncChildren(document.getElementById('sessionList'),sessions,item=>`session-${item.route}-${item.id}`,sessionCard,'No session history');}
async function refreshWorkbench(){const requestedPage=historyPage,res=await fetch(`/api/workbench?page=${encodeURIComponent(requestedPage)}`,{cache:'no-store'}),data=await res.json();if(!res.ok||!data.ok)throw new Error(data.error||'Workbench failed to load');if(requestedPage!==historyPage)return data;storeWorkbench(data);renderWorkbench(data);return data;}
