...
OWNER_STATIC_FILES = {"appearance.css", "appearance.js", "app.js", "style.css", "index.html", "event-stream.js", "internal-annotations.js", "local-file-view.js", "stable-blocks.js", "question-navigator.js", "live-scroll.js", "compact-rules-codex.js", "codex-commands.js", "copy-fidelity.js"}
...
function resetSheetMode(){const modal=document.getElementById('modal'),back=document.getElementById('modalBack'),toolbar=document.getElementById('directoryToolbar'),breadcrumb=document.getElementById('directoryBreadcrumb'),search=document.getElementById('directorySearch');modal.classList.remove('directory-mode');back.hidden=true;back.onclick=null;toolbar.hidden=true;breadcrumb.replaceChildren();search.value='';search.oninput=null;}
...
function directoryPickerModel(data,recent,query,expanded){const search=String(query||'').trim().toLowerCase(),current=directoryCanonical(data.path,data),parent=directoryCanonical(data.parent,data),roots=(data.roots||[]).map(item=>({...item,canonical:directoryCanonical(item.path,data)})),reserved=new Set([current,parent,...roots.map(item=>item.canonical)].filter(Boolean)),seen=new Set(reserved),allRecent=[];for(const item of recent||[]){const value=String(item.value||item.path||''),canonical=directoryCanonical(value,data);if(!canonical||seen.has(canonical))continue;seen.add(canonical);allRecent.push({kind:'recent',icon:'↺',label:item.label||directoryName(canonical),meta:item.path||value,path:value});}const locations=roots.filter(item=>item.canonical&&item.canonical!==current&&!current.startsWith(item.canonical+'/')).map(item=>({kind:'location',icon:'⌂',label:item.displayPath||directoryName(item.canonical),meta:'Configured location',path:item.path})),folders=(data.directories||[]).filter(item=>{const canonical=directoryCanonical(item.path,data);return canonical&&!seen.has(canonical);}).map(item=>({kind:'folder',icon:'📁',label:item.name||directoryName(item.path),meta:'',path:item.path}));const matches=item=>!search||`${item.label} ${item.meta}`.toLowerCase().includes(search),recentMatches=allRecent.filter(matches),recentVisible=(search||expanded?recentMatches:recentMatches.slice(0,4)),locationVisible=locations.filter(matches),folderVisible=folders.filter(matches);return{recent:recentVisible,locations:locationVisible,folders:folderVisible,hasMore:!search&&allRecent.length>4,total:recentVisible.length+locationVisible.length+folderVisible.length};}
...
async function refreshWorkbench(){const requestedPage=historyPage,data=await fetchJson(`/api/workbench?page=${encodeURIComponent(requestedPage)}`,{cache:'no-store'},'Workbench');if(requestedPage!==historyPage)return data;storeWorkbench(data);renderWorkbench(data);return data;}
