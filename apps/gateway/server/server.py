...
OWNER_STATIC_FILES = {"appearance.css", "appearance.js", "app.js", "style.css", "index.html", "event-stream.js", "internal-annotations.js", "local-file-view.js", "stable-blocks.js", "question-navigator.js", "live-scroll.js", "compact-rules-codex.js", "compact-rules-claude.js"}
...
class GatewayHandler(BaseHTTPRequestHandler):
    ...
    def handle_agent_new(self, username: str) -> None:
        try:
            ...
            recent_cwd = select_recent_agent_cwd([*recent["activeSessions"], *recent["sessions"]], self.config.workspace_root(username, route))
    ...
    def workbench_payload(self, username: str, history_page: int = 1) -> dict[str, Any]:
        ...
        return {
            ...
            "newSessionCommands": sorted(NEW_SESSION_COMMANDS if username == self.config.mcp_user else {"codex"}),
            "packages": inbox,
            "inbox": inbox,
            "updatedAt": now_ts(),
        }
...
async function agentNew(route,command){const res=await fetch('/api/agent/new',{method:'POST',headers:{'Content-Type':'application/json',...(await csrfHeaders())},body:JSON.stringify({route,command})}),data=await res.json();if(!res.ok||!data.ok)throw new Error(data.error||'Failed to create session');location.href=data.redirect;}
