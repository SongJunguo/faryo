...
OWNER_STATIC_FILES = {"appearance.css", "appearance.js", "app.js", "style.css", "index.html", "math-render.js", "markdown-render.js", "compact-rules-codex.js", "compact-rules-claude.js"}
...
main{gap:16px}.session-section{display:grid;gap:8px;min-width:0}.history-list{max-height:min(62vh,720px);overflow-y:auto;overscroll-behavior:contain;scrollbar-gutter:stable;padding-right:2px}.history-pager{display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:8px}.history-pager .count{text-align:center}.history-pager button:disabled{opacity:.42}@media(max-width:620px){.history-list{max-height:58vh}}
...
const WORKBENCH_CACHE_KEY='faryoWorkbenchSnapshot';const labels=__LABELS_JS__;let draggedPackage=null;let touchDrag=null;let assetTargetPackage=null;let actionBusy=false;let historyPage=1;
...
function renderWorkbench(data){markRoutes(data.entries||[]);const packages=data.inbox||data.packages||[],rawSessions=data.sessions||[],activeSessions=Array.isArray(data.activeSessions)?data.activeSessions:rawSessions.filter(item=>item.tmuxSession),sessions=Array.isArray(data.activeSessions)?rawSessions:rawSessions.filter(item=>!item.tmuxSession),history=data.history||{},entries=data.entries||[],pkg=packages[0],packageItems=pkg?[pkg]:[],allowedCommands=new Set(data.newSessionCommands||['codex']),launchers=[{id:'new-codex',command:'codex',label:'Codex CLI',entries},{id:'new-claude',command:'claude',label:'Claude Code',entries}].filter(item=>allowedCommands.has(item.command));historyPage=Math.max(1,Number(history.page||historyPage||1));document.getElementById('packageCount').textContent=pkg?(pkg.status==='pending'?'· New':'· Done'):'· Empty';document.getElementById('activeSessionCount').textContent=`${activeSessions.length} live`;document.getElementById('historyCount').textContent=`${Number(history.total??sessions.length)} total`;document.getElementById('historyPageLabel').textContent=`Page ${historyPage} of ${Math.max(1,Number(history.totalPages||1))}`;document.getElementById('historyPrev').disabled=history.hasPrevious===false||historyPage<=1;document.getElementById('historyNext').disabled=history.hasNext===false;syncChildren(document.getElementById('packageList'),packageItems,item=>`pkg-${item.id}`,packageCard,'No handoff package');syncChildren(document.getElementById('newSessionSlot'),launchers,item=>item.id,newAgentCard,'');syncChildren(document.getElementById('activeSessionList'),activeSessions,item=>`active-${item.route}-${item.tmuxSession||item.id}`,sessionCard,'No active agent sessions');syncChildren(document.getElementById('sessionList'),sessions,item=>`session-${item.route}-${item.id}`,sessionCard,'No session history');}
async function refreshWorkbench(){const res=await fetch(`/api/workbench?page=${encodeURIComponent(historyPage)}`,{cache:'no-store'}),data=await res.json();if(!res.ok||!data.ok)throw new Error(data.error||'Workbench failed to load');storeWorkbench(data);renderWorkbench(data);return data;}
async function changeHistoryPage(delta){const previous=historyPage,next=Math.max(1,previous+delta);if(next===previous)return;historyPage=next;try{await refreshWorkbench();document.getElementById('sessionList')?.scrollTo({top:0,behavior:'smooth'});}catch(error){historyPage=previous;throw error;}}
