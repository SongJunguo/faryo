...
MCP_DEVFS_TOOLS = {"faryo_list_dir", "faryo_read_file", "faryo_search_text", "faryo_write_file", "faryo_replace_text", "faryo_mkdir", "faryo_move_path", "faryo_delete_path", "faryo_git_status", "faryo_git_diff"}
MCP_TASK_TOOLS = {"faryo_run_tests", "faryo_run_build", "faryo_run_lint"}
...
class GatewayHandler(BaseHTTPRequestHandler):
    ...
    def mcp_response(self, payload: Any) -> dict[str, Any] | list[dict[str, Any]] | None:
        ...
        try:
            ...
            if method == "tools/call":
                ...
                if name in MCP_DEVFS_TOOLS: return self.mcp_result(request_id, self.mcp_devfs(name, arguments))
                if name in MCP_TASK_TOOLS: return self.mcp_result(request_id, self.mcp_task(name, arguments))
    ...
    def mcp_devfs(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"structuredContent": result, "content": [{"type": "text", "text": json.dumps(result, ensure_ascii=False)}], "_meta": {}}
    ...
    def require_mcp_token(self) -> bool:
        ...
        if auth.lower().startswith("bearer "):
            token = auth[7:].strip()
        if token and hmac.compare_digest(token, self.config.mcp_token):
            return True
    ...
    def owner_agent_sessions(self, route: str, username: str, history_mode: str = "less") -> dict[str, Any]:
        ...
        for item in raw_sessions:
            ...
            sessions.append({"id": str(item.get("id") or ""), "title": display_session_title(item.get("title") or item.get("label") or item.get("id") or "Untitled session"), "gitLabel": str(item.get("gitLabel") or item.get("git_label") or ""), "route": route, "routeLabel": BACKENDS[route][2], "cwd": cwd, "cwdLabel": compact_path_label(cwd), "updatedAt": display_updated_at(updated_raw), "updatedTs": float(item.get("updatedTs") or parse_updated_ts(updated_raw)), "tmuxSession": tmux_session, "active": active, "agentRunning": bool(active and item.get("agentRunning")), "agentState": str(item.get("agentState") or ""), "limitReached": (not active and limit_reached), "source": str(item.get("source") or "")})
